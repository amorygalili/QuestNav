...
namespace QuestNav.Core
{
    ...
    public class QuestNav : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // Ensure we have an AppLifecycleManager
            if (FindObjectOfType<AppLifecycleManager>() == null)
            {
                GameObject lifecycleManager = new GameObject("AppLifecycleManager");
                lifecycleManager.AddComponent<AppLifecycleManager>();
                Debug.Log("[QuestNav] Created AppLifecycleManager");
            }
